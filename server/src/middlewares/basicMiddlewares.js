...
const { USER_ROLES, CONTEST_STATUSES } = require('../constants');
...
module.exports.canGetContest = async (req, res, next) => {
  let result = null;
  try {
    if (req.tokenData.role === USER_ROLES.CUSTOMER) {
      result = await db.Contest.findOne({
        where: { id: req.headers.contestid, userId: req.tokenData.userId },
      });
    } else if (req.tokenData.role === USER_ROLES.CREATOR) {
      result = await db.Contest.findOne({
        where: {
          id: req.headers.contestid,
          status: {
            [ db.Sequelize.Op.or ]: [
              CONTEST_STATUSES.CONTEST_STATUS_ACTIVE,
              CONTEST_STATUSES.CONTEST_STATUS_FINISHED,
            ],
          },
        },
      });
...
module.exports.onlyForCustomer = (req, res, next) => {
  if (req.tokenData.role === USER_ROLES.CREATOR) {
  ...
  try {
    const result = await db.Contest.findOne({
      where: {
        ...
        id: req.body.contestId,
        status: CONTEST_STATUSES.CONTEST_STATUS_ACTIVE,
      },
    });
    if (!result) {
      return next(new RightsError());
  ...
  try {
    const result = db.Contest.findOne({
      where: {
        userId: req.tokenData.userId,
        id: req.body.contestId,
        status: { [ db.Sequelize.Op.not ]: CONTEST_STATUSES.CONTEST_STATUS_FINISHED },
      },
    });
    if (!result) {
      return next(new RightsError());
